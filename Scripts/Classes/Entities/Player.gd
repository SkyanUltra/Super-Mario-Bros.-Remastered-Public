...
func add_stomp_combo(award_score := true) -> void:
	if stomp_combo >= 10:
		if award_score:
			if [Global.GameMode.CHALLENGE, Global.GameMode.BOO_RACE, Global.GameMode.PHANTO_PURSUIT].has(Global.current_gamemode) or Settings.file.difficulty.inf_lives:
				Global.score += 10000
